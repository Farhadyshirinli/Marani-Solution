...
namespace Marani.Domain.Business.ProductModule
{
    public class ProductEditCommand : IRequest<Product>
    {
        ...
        public class ProductEditCommandHandler : IRequestHandler<ProductEditCommand, Product>
        {
            ...
            public ProductEditCommandHandler(MaraniDbContext db, IHostEnvironment env, IActionContextAccessor ctx)
            {
                this.db = db;
                this.env = env;
                this.ctx = ctx;
            ...
            public async Task<Product> Handle(ProductEditCommand request, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    var data = await db.ProductCatalog.FirstOrDefaultAsync(p =>
                    ...
                    if (request.Images != null && request.Images.Count() > 0)
                    {
                        ...
                        {
                            var productImage = await db.ProductsImages.FirstOrDefaultAsync(pi => pi.Id == item.Id && pi.ProductId == model.Id && pi.DeletedByUserId == null);
